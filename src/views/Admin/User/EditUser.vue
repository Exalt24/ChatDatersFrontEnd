<template>
    <div v-if="user" :id="`user-${user.id}`" class="max-w-md mx-auto mt-8 mb-8">
...
<div class="form-group">
        ...
        <div class="form-group">
          ...
          <div v-if="form.existingImages.length > 0" class="flex flex-wrap">
            <div v-for="(imageUrl, index) in form.existingImages" :key="index" class="my-2 w-1/4 relative">
              ...
              <button type="button" @click="deleteImage(imageUrl)" class="absolute top-1 right-1 px-2 py-1 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs">
                <span>&times;</span>
              </button>
            </div>
          </div>
        ...
        <div class="flex justify-between mt-4">
          ...
          <div class="flex">
            <router-link :to="{ name: 'Show User', params: { id: user.id } }" class="px-4 py-2 bg-gray-200 text-gray-900 rounded-md hover:bg-[#637575]">Show</router-link>
            <div class="ml-2">
              <button type="submit" class="px-4 py-2 bg-gray-900 text-white rounded-md mr-4 hover:bg-[#637575]">Update</button>
            </div>
          </div>
        </div>
      </Form>
